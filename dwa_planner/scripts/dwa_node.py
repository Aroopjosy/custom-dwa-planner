...
class DWAPlannerNode(Node):
    ...
    def __init__(self):
        ...
        self.w_heading   = 1.0  # less bias toward goal
        self.w_clearance = 3.0   # strong obstacle avoidance
        self.w_velocity  = 0.1   # prefers moving, but not too much
        self.w_smooth    = 0.1   # prevents jerky motions
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        for r in msg.ranges:
            ...
            nearest_dist = min(nearest_dist, r)

            angle += msg.angle_increment
            # Debug log
        if self.obstacles:
            self.get_logger().info(f"Nearest obstacle: {nearest_dist:.2f} m")
        else:
    ...
    def control_loop(self):
        ...
        for v in vs:
            for w in ws:
                ...
                # if clearance == 0.0:
                #     self.get_logger().info(f"Trajectory  → COLLISION predicted ❌")
                # else:
                #     self.get_logger().info(f"Trajectory  → clearance={clearance:.2f}")

                if clearance < 0.05 or not stopping_feasible(v, self.a_max, self.dt_control, clearance, self.effective_radius):
                    continue
                ...
                # self.get_logger().info(f"clearance score {clearance_score(clearance, self.max_clearance)}")
        ...
        if candidates:
            ...
            top_candidates = [traj for traj, _ in candidates[:30]]
